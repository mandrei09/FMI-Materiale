...
--- | Satisface predicatul dat ca parametru
...
runParser (Parser p) input = case p input of
    [] -> Left "Parser failed"
    [(x,rest)] -> Right (x, rest)
    _ -> Left "Parser ambiguous"

parse :: Parser a -> String -> Either String a
parse p input = case runParser (p <* endOfInput) input of
  Left err -> Left "Sirul de intrare nu a fost complet consumat sau parsare ambigua"
  Right (x, _) -> Right x
